...
public class NioSocketChannel extends AbstractNioByteChannel implements io.netty.channel.socket.SocketChannel {
    ...
    private void adjustMaxBytesPerGatheringWrite(int attempted, int written, int oldMaxBytesPerGatheringWrite) {
        ...
        if (attempted == written) {
            ...
            // written < attempted >>> 1 :写入的数据还不及尝试写入数据的 1 / 2，说明当前容量不是很够了
            ((NioSocketChannelConfig) config).setMaxBytesPerGatheringWrite(attempted >>> 1);
        }
    }
    ...
    protected void doWrite(ChannelOutboundBuffer in) throws Exception {
        ...
        /**
         * writeSpinCount < 0
         * : case 0 中传输文件的时候，假如Socket缓冲区已满，
         * doWrite0会返回Integer.MAX_VALUE，所以会出现 writeSpinCount < 0
         *
         * writeSpinCount == 0
         *  以写满16次，但是还没写完，Socket缓冲区还可以继续写
         */
        ...
        protected Executor prepareToClose() {
            try {
                if (javaChannel().isOpen() && config().getSoLinger() > 0) {
                    ...
                    // 在设置SO_LINGER后，channel会延时关闭，在延时期间我们仍然可以进行读写，
                    // 这样会导致io线程eventloop不断的循环浪费cpu资源
                    // 所以需要在延时关闭期间 将channel注册的事件全部取消。
                    doDeregister();
                    /**
                     * 设置了SO_LINGER,不管是阻塞socket还是非阻塞socket，
                     * 在关闭的时候都会发生阻塞，所以这里不能使用Reactor线程来
