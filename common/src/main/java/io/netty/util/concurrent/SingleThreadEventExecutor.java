...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
    ...
    /**
     * addTaskWakesUp 初始化为 false 。
     * 表示并不是只有 addTask 方法才能唤醒 Reactor 线程 还有其他方法可以唤醒 Reactor 线程，
     * 比如 SingleThreadEventExecutor#execute 方法
     * 还有本小节介绍的 SingleThreadEventExecutor#shutdownGracefully 方法都会唤醒 Reactor 线程。
     */
    ...
    protected SingleThreadEventExecutor(EventExecutorGroup parent, Executor executor,
        ...
        // 用于驱动Reactor线程的executor -> ThreadPerTaskExecutor
        this.executor = ThreadExecutorMap.apply(executor, this);
        // 普通任务队列
    ...
    public Future<?> shutdownGracefully(long quietPeriod, long timeout, TimeUnit unit) {
        ...
        int oldState;
        for (;;) {
            // for 循环中通过 CAS 不停的尝试将 Reactor 的当前 ST_STARTED 状态改为 ST_SHUTTING_DOWN 正在关闭状态
            if (isShuttingDown()) {
                return terminationFuture();
            }
            int newState;
            ...
            oldState = state;
            if (inEventLoop) {
                // 通过 inEventLoop() 判断出当前执行线程是 Reactor 线程，
                // 那么表示当前 Reactor 的状态只会是 ST_STARTED 运行状态，
                // 那么就可以直接将 newState 设置为 ST_SHUTTING_DOWN 。
                // 因为只有 Reactor 处于 ST_STARTED 状态的时候才会运行到这里。
                // 否则在前边就直接返回 terminationFuture 了
                newState = ST_SHUTTING_DOWN;
            } else {
                // 如果不是reactor线程而是用户线程，则有可能会发起重复的关闭reactor流程，
                // 所以需要判断当前是否处于关闭状态
                switch (oldState) {
                    case ST_NOT_STARTED:
                    case ST_STARTED:
        ...
        // 优雅关闭静默期，在该时间内，用户还是可以向Reactor提交任务并且执行，
        // 只要有任务在Reactor中，就不能进行关闭
        // 每隔100ms检测是否有任务提交进来，如果在静默期内没有新的任务提交，
        // 那么才会进行关闭 保证关闭行为的优雅
        gracefulShutdownQuietPeriod = unit.toNanos(quietPeriod);
        // 优雅关闭的最大超时时间，优雅关闭行为不能超过该时间
        // 如果超过的话 不管当前是否还有任务 都要进行关闭 保证关闭行为的可控
     ...
     * 将剩余的异步任务执行完毕。在该方法中只要有异步任务需要执行，就不能关闭，保证业务无损。
        ...
        executor.execute(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    // 完成 Reactor 的最终关闭流程，也就是 Reactor 在退出 run 方法中的 for 循环之后的后续收尾流程
